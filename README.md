...
- Two optional selectable sensors for fan speed and CPU load, showing as arcs on sides
...
- In current version of iCUE (5.47), only the currently selected widget of a profile has persistent storage for its customizable values, which means all non-selected widgets (including pre-installed native widgets) will restore to defaults when switching away from the profile or closing iCUE. Keep the widget selected before leaving iCUE or switching profile to prevent loss of settings. 
